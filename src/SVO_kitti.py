...
if __name__ == "__main__":
    ...
    datapath = '/home/sysop/kitti/dataset/sequences/00'
    #
...
for frm in range(startFrame + 1, endFrame + 1):
    ...
    imgPath = leftImagePath + f'{frm:06}.png'
    ImT1_L = cv2.imread(imgPath, 0)  # 0 flag returns a grayscale image

    imgPath = rightImagePath + f'{frm:06}.png'
    ImT1_R = cv2.imread(imgPath, 0)

    imgPath = leftImagePath + f'{frm + 1:06}.png'
    ImT2_L = cv2.imread(imgPath, 0)

    imgPath = rightImagePath + f'{frm + 1:06}.png'
    ...
    if plotTrajectory:
        ...
        cv2.circle(traj, (draw_x, draw_y), 3,
                   (frm * 255 / (endFrame - startFrame), 255 - frm * 255 / (endFrame - startFrame), 0), 3)
